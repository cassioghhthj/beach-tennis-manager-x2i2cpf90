...
BEGIN
  ...
  FOR v_atleta IN (
    ...
    IF EXISTS (SELECT 1 FROM public.pontuacoes_rodada WHERE rodada_id = p_rodada_id AND atleta_id = v_atleta.atleta_id) THEN
      ...
      -- A função COALESCE garante que os pontos manuais e de pódio nunca sejam zerados durante o recálculo
      ...
      SET
        ...
        pontos_manuais = COALESCE(pontos_manuais, 0),
        pontos_podio_principal = COALESCE(pontos_podio_principal, 0),
        pontos_podio_consolacao = COALESCE(pontos_podio_consolacao, 0)
