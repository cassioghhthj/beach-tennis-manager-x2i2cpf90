...
ALTER TABLE public.pontuacoes_rodada ADD COLUMN IF NOT EXISTS vitorias integer NOT NULL DEFAULT 0;
ALTER TABLE public.pontuacoes_rodada ADD COLUMN IF NOT EXISTS derrotas integer NOT NULL DEFAULT 0;
ALTER TABLE public.pontuacoes_rodada ADD COLUMN IF NOT EXISTS games_pro integer NOT NULL DEFAULT 0;
ALTER TABLE public.pontuacoes_rodada ADD COLUMN IF NOT EXISTS games_contra integer NOT NULL DEFAULT 0;
ALTER TABLE public.pontuacoes_rodada ADD COLUMN IF NOT EXISTS saldo_games integer NOT NULL DEFAULT 0;

-- Update the function to calculate and save the new statistics
...
-- Backfill existing data dynamically using DO block
DO $$
DECLARE
  v_rodada record;
BEGIN
  -- Process any rounds that are not in Draft status to populate the new statistics
  FOR v_rodada IN (SELECT id FROM public.rodadas WHERE status != 'Draft') LOOP
    PERFORM public.processar_presenca_rodada(v_rodada.id);
  END LOOP;
END $$;
