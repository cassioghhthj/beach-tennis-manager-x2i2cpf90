DO $DO$
BEGIN
  -- Create the trigger function for calculating the total correctly
END $DO$;
...
DROP TRIGGER IF EXISTS trg_calcular_total_pontuacoes ON public.pontuacoes_rodada;
CREATE TRIGGER trg_calcular_total_pontuacoes
BEFORE INSERT OR UPDATE ON public.pontuacoes_rodada
FOR EACH ROW
EXECUTE FUNCTION public.calcular_total_pontuacoes();
...
BEGIN
  ...
  -- Processar para todos os atletas de todos os grupos da rodada E os que ja tem pontuacao
  FOR v_atleta IN (
    ...
    IF EXISTS (SELECT 1 FROM public.pontuacoes_rodada WHERE rodada_id = p_rodada_id AND atleta_id = v_atleta.atleta_id) THEN
      -- Atualiza os pontos de presença, novas estatísticas. (O trigger calculará o total automaticamente)
...
-- Atualiza as pontuações já existentes para garantir que o trigger dispare e recalcule o total
UPDATE public.pontuacoes_rodada SET id = id;
